...
# Hàm kiểm tra FVM có cài đặt không
check_fvm() {
...
# Hàm lấy danh sách Flutter versions từ FVM
get_fvm_versions() {
    if ! check_fvm; then
        return 1
    fi
    fvm releases 2>/dev/null | grep -E "^[0-9]+\.[0-9]+\.[0-9]+" | awk '{print $1}' | sort -V
}

# Hàm lấy Flutter version hiện tại
get_current_flutter_version() {
    if check_fvm && [ -f ".fvm/fvm_config.json" ]; then
        # Lấy version từ FVM config
        if command -v jq &> /dev/null; then
            jq -r '.flutterSdkVersion // empty' .fvm/fvm_config.json 2>/dev/null
        else
            grep -o '"flutterSdkVersion": "[^"]*"' .fvm/fvm_config.json 2>/dev/null | cut -d'"' -f4
        fi
    elif command -v flutter &> /dev/null; then
        flutter --version 2>/dev/null | head -n 1 | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n 1
    fi
}
...
show_help() {
    ...
    echo "  --flutter-version=<ver>    Flutter version to use (e.g., 3.24.0)"
    echo "  --fvm                      Use FVM (Flutter Version Management)"
    echo "  --fvm=<ver>                Use FVM with specific version (e.g., --fvm=3.24.0)"
    ...
    echo "  $0 myapp --fvm=3.24.0"
    echo "  $0 myapp --flutter-version=3.24.0"
...
# 7. Cấu hình Flutter version (FVM) nếu được yêu cầu
if [ -n "$FLUTTER_VERSION" ] || [ "$USE_FVM" = true ]; then
    print_step "Cấu hình Flutter version"
    
    if ! check_fvm; then
        print_warning "FVM chưa được cài đặt, bỏ qua cấu hình Flutter version"
        print_info "Để cài đặt FVM:"
        print_info "  dart pub global activate fvm"
        print_info "Hoặc:"
        print_info "  brew tap leoafarias/fvm"
        print_info "  brew install fvm"
    else
        cd "$NEW_PROJECT_DIR"
        
        # Nếu chỉ dùng --fvm mà không chỉ định version, dùng version từ template
        if [ -z "$FLUTTER_VERSION" ] && [ "$USE_FVM" = true ]; then
            if [ -f "$TEMPLATE_DIR/.fvm/fvm_config.json" ]; then
                if command -v jq &> /dev/null; then
                    FLUTTER_VERSION=$(jq -r '.flutterSdkVersion // empty' "$TEMPLATE_DIR/.fvm/fvm_config.json" 2>/dev/null)
                else
                    FLUTTER_VERSION=$(grep -o '"flutterSdkVersion": "[^"]*"' "$TEMPLATE_DIR/.fvm/fvm_config.json" 2>/dev/null | cut -d'"' -f4)
                fi
            fi
            
            if [ -z "$FLUTTER_VERSION" ]; then
                # Lấy version hiện tại của Flutter system
                FLUTTER_VERSION=$(flutter --version 2>/dev/null | head -n 1 | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n 1)
                print_substep "Sử dụng Flutter version hiện tại: $FLUTTER_VERSION"
            fi
        fi
        
        if [ -n "$FLUTTER_VERSION" ]; then
            print_substep "Sử dụng Flutter version: $FLUTTER_VERSION"
            
            # Kiểm tra version có trong FVM không
            if ! fvm releases 2>/dev/null | grep -qE "^[[:space:]]*$FLUTTER_VERSION[[:space:]]"; then
                print_substep "Version $FLUTTER_VERSION chưa có trong FVM, đang cài đặt..."
                if fvm install "$FLUTTER_VERSION" 2>/dev/null; then
                    print_success "Đã cài đặt Flutter $FLUTTER_VERSION"
                else
                    print_warning "Không thể cài đặt Flutter $FLUTTER_VERSION, sử dụng version có sẵn"
                fi
            fi
            
            # Cấu hình FVM cho project
            if fvm use "$FLUTTER_VERSION" --force 2>/dev/null; then
                print_success "Đã cấu hình FVM với Flutter $FLUTTER_VERSION"
            else
                print_warning "Không thể cấu hình FVM"
            fi
        fi
